...
**`cucli tasks <team_id>`** - List tasks with filters: `--status`, `--space-id`, `--list-id`, `--assignee`, `--tag`, `--include-closed`, `--page`
...
**`cucli add-dependency <task_id>`** - Add dependency: `--depends-on <id>` or `--dependency-of <id>`

**`cucli delete-dependency <task_id>`** - Remove dependency: `--depends-on <id>` or `--dependency-of <id>`
...
**`cucli update-checklist <checklist_id>`** - Update checklist: `--name`, `--position` (0 = top)
...
**`cucli create-key-result <goal_id> --name "Key Result Name"`** - Create a key result (supports `--type` [number|currency|boolean|percentage|automatic], `--unit`, `--steps-start`, `--steps-end`, `--owner`, `--task`, `--list`)
